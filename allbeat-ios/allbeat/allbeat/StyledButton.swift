...
//  allbeat
//
//  Created by Nathan Flurry on 12/17/15.
//  Copyright © 2015 allbeat, LLC. All rights reserved.
//

import UIKit

@IBDesignable class StyledButton: UIControl {
	...
	func press() {
		// Darken the color
		let pressColorOffset: CGFloat = 0.2
		backgroundColor = background.offsetColors(-pressColorOffset)
		tintColor = tint.offsetColors(-pressColorOffset)
	}
	...
	func depress() {
		...
		UIView.animate(
			withDuration: 0.4,
			delay: 0,
			options: UIViewAnimationOptions(rawValue: UIViewAnimationOptions().rawValue | UIViewAnimationOptions.allowUserInteraction.rawValue),
			animations: {
