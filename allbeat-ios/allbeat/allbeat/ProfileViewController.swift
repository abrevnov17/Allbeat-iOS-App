...
class ProfileViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if(FIRAuth.auth()?.currentUser != nil){
            ...
            //setting pro pic
            Allbeat.getUserProPic(userID: (FIRAuth.auth()?.currentUser?.uid)!) { (url) in
                if (url != nil){
            ...
            Allbeat.aggregatePosts(userID: Allbeat.getCurrentUser().uid, num: 100) { (posts: Array<String>?) in
                if (posts != nil) {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        // }
        Allbeat.getNumFollowers(userID: (user1.uid)) { (followers) in
            if (followers != nil){
        ...
        Allbeat.getNumFollowing(userID: (user1.uid)) { (following) in
            if (following != nil){
        ...
        Allbeat.getUserName(userID: (user1.uid)) { (name) in
            self.username.text = "@" + name!
        }
        
        Allbeat.getUserDisplayName(userID: user1.uid) { (name) in
            if (name != nil){
                self.ArtistName.text = name!
            }
            else {
        ...
        Allbeat.getUserBio(userID: (user1.uid)) { (bio) in
            if (bio != nil){
                self.bio.text = bio!
            }
            else {
...
extension ProfileViewController: AudioClipCellDelegate {
...
extension ProfileViewController: AudioPlayerDelegate {
